...
const StockRequest = ({ draftsOnly = false }) => {
  ...
  const applyFilters = (items) => {
    return items.filter(item => {
      ...
      if (filters.staff && !item.requested_by_name?.toLowerCase().includes(filters.staff.toLowerCase()) && !item.class_name?.toLowerCase().includes(filters.staff.toLowerCase())) return false;
      return true;
    });
  };
  ...
  // ===== RENDER =====
  const showLoadingSpinner = loading && requests.length === 0;
      ...
      <div className="cr-page animate-up">
        <div className="sr-title-row">
          ...
          <div className="sr-title-right">
            {pendingCount > 0 && <span className="cr-count-badge cr-count-blue">{pendingCount} Pending</span>}
            {rejectedCount > 0 && <span className="cr-count-badge cr-count-red">{rejectedCount} Rejected</span>}
            <span className="cr-count-badge cr-count-gray">{allCount} Total</span>
          </div>
        </div>
        ...
        {showFilters && (
          <div className="sr-filters-panel animate-fade">
            <div className="sr-filters-grid">
              ...
              <div className="sr-filter-group">
                <label>Class:</label>
                <input type="text" placeholder="Class name..." value={filters.staff} onChange={(e) => handleFilterChange('staff', e.target.value)} />
              </div>
            </div>
        ...
        {showLoadingSpinner ? (
          ...
          <>
            <div className="sr-card-list-desktop">
              {filtered.map(req => (
                <div key={req.id} className={`sr-card ${req.status === 'rejected' ? 'sr-card-rejected' : req.status === 'pending' ? 'sr-card-pending' : ''}`} onClick={() => navigate(`/requests/${req.id}`)}>
                  ...
                  <div className="sr-card-info">
                    ...
                    <div className="sr-card-class">{req.status.charAt(0).toUpperCase() + req.status.slice(1)}</div>
                  </div>
            ...
            <div className="sr-card-list-mobile">
              {filtered.map(req => (
                <div key={req.id} className={`sr-card-mobile ${req.status === 'rejected' ? 'sr-card-mobile-rejected' : req.status === 'pending' ? 'sr-card-mobile-pending' : ''}`} onClick={() => navigate(`/requests/${req.id}`)}>
                  ...
                  <div className="sr-mobile-meta">{req.status.charAt(0).toUpperCase() + req.status.slice(1)} · 📅 {formatDate(req.date)}</div>
                </div>
              ))}
            </div>
          </>
          ...
          <div className="sr-empty">
            ...
            <div className="sr-empty-sub">Your chemical requests will appear here</div>
          </div>
        )}
